...
//   cin.ignore();
   ...
   //cin.getline(sendDataBuf,maxDataSize);
      ...
      //getline(cin,sendDataStr);
      dataSent = sendto(clientSocketFd, sendDataStr.c_str(), sendDataStr.length(), flags, (struct sockaddr *)&serverSockAddressInfo, sizeof(struct sockaddr));
     ...
     // cout<<"Is data sent successfully"<<dataSent<<endl;
      ...
      dataRecvd = recvfrom(clientSocketFd, &rcvDataBuf, maxDataSize, flags,(struct sockaddr *)&clientAddressInfo, &sinSize);
